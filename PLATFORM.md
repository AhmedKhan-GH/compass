...
| **Signal Workbench** *(flagship — built on macOS, I3)* | EDF/WFDB recordings + annotation sidecar files | AUI docking (channel tree ⋮ waveform canvas ⋮ annotation table); wxTreeCtrl lead/channel tree; wxDataViewCtrl annotation list synced to the canvas; command-stack undo spanning both; native file dialogs + drag-and-drop of recordings | `Canvas2D` (decimated min/max waveforms) | I3 |
